...
#   sudo dnf install git redhat-lsb-core glibc-devel.aarch64 glibc-headers-s390.noarch glibc-headers-x86.noarch glibc-common.aarch64 glibc-static gcc make -y
    sudo xbps-install -yv git rpi4-kernel-headers gcc glibc glibc-devel make
}
...
deskpidaemon=/etc/sv/$daemonname/$daemonname
safeshutdaemon=/etc/sv/$daemonname/$daemonname-safeshut
...
sudo ln -s /etc/sv/$deskpidaemon /var/service/ &
sudo ln -s /etc/sv/$deskpidaemon-safeshut /var/service/
